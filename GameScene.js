...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    this.load.image('guy-front', 'https://i.ibb.co/7tr3PCC/14.png');
    this.load.image('guy-back', 'https://i.ibb.co/sycQyh7/16.png');
    this.load.image('guy-side', 'https://i.ibb.co/s6gbs9n/15.png');
    this.load.image('gem', 'https://i.ibb.co/DLYgyzS/13.png');
    this.load.image('glass', 'https://i.ibb.co/9vRsZkB/14.png');
  }
  create() {
    ...
    this.physics.add.overlap(gameState.player, gameState.gem, () => {
      ...
      score += (Math.round(Math.random() * 10) * gemMultiplier);
      scoreText.setText(`Earnings: \💎+${score}`);
      randomCoord = assignCoords();
      gameState.enemies.create(randomCoord.x, randomCoord.y, 'glass').setScale(.6);
    });
